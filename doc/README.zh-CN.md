...
- 提交流程与 Phase 1 类似，但请注意提交接口和输出接口已经发生变化：

  * `/submit` -> `/submit3`
  * `/outputs` -> `/outputs3`
  * `/submit_status` 保持不变

- 本阶段不提供 `/submit-test`。你现在可以使用 `/submit2` 中的 deepseek API key，或者直接使用自己的 API key。
...
- 评测系统会先运行你的 agent。当你的 agent 完成或超时后，系统会收集生成的：

```bash
/workspace/engine.py
```

- 如果你的 agent 没有生成 `result.log` 文件，评测系统会自动生成该文件用于调试。
